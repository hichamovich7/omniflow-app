...
TASK-FIX-044 — WordPress P0 quality: make the FAQ visible and stop the external-link step from rewriting the article.

### Context

The 2026-07-15 decision kept the FAQ only in the structured `faq` field ("jamais en texte libre dans le Markdown"), but that field was never persisted or rendered, so every generated FAQ was discarded. `addExternalLink()` asked the model to echo the whole article back with one link inserted, which let it silently rewrite content and cost ~9000 output tokens.

### Decision Taken

1. The article writes a `{{FAQ}}` line (after Common Mistakes, before the Conclusion); `insertFaqSection()` renders the structured `faq` there as one localized H2 section — the visible FAQ and the structured one cannot diverge. No migration: the structured field stays in the validated response for a later FAQPage/persistence task.
2. `addExternalLink()` now receives only an anchor phrase and a source from the model and inserts the link server-side on that exact phrase (prose lines only), after the existing HTTP check. The article is never regenerated by that step.
3. Pins: the primary keyword is the Pinterest generation's `generations.keyword`; the frequency heuristic is a fallback only.

### Consequences

* Supersedes the "FAQ never in the Markdown" part of the 2026-07-15 decision and the "retourner l'article complet" part of the external-link decision.
* Articles generated before this change keep no visible FAQ.
