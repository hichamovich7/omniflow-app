...
6. **Content streams** — every non-archived stream: stream + project, Pinterest board, targets ("5 Pins/day · 5-day buffer"), planned / required, last planned date, days covered, status badge (On track · Needs content · Create now · Warming · Paused · Needs setup) and one action link. Table on desktop (`md+`), stacked cards on mobile.
7. **Publishing coverage** — four separate counts: *Created* (all pins), *Planned* (date still ahead), *Planned date passed* (explicitly "not verified on Pinterest"), *Unscheduled*. Then a 14-day grid per stream (filled = target met, light = below target, grey = nothing, amber outline = inside the buffer window and needs new Pins) and one sentence per stream, e.g. "No urgent work for Bathroom Ideas. You are covered through October 5."
