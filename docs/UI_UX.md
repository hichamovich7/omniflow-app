...
7. **Publishing coverage** — six separate counts: *Created* (all pins), *Planned* (date still ahead), *Planned date passed* (explicitly "not verified on Pinterest"), *Unscheduled*, *Confirmed externally* (today, entered manually — TASK-FIX-043) and *Expected externally* (next 14 days, not confirmed — TASK-FIX-053). Then a 14-day grid per stream (filled = target met, light = below target, grey = nothing, amber outline = inside the buffer window and needs new Pins, green dot = Pins published externally, hollow dashed dot = Pins expected externally) and one sentence per stream, e.g. "No urgent work for Bathroom Ideas. You are covered through October 5." Paused and planned streams get no row.
   * **Publishing activity modal** (TASK-FIX-043): today's cell is a button (hover ring, keyboard-focusable) that opens a `Dialog` — "Publishing activity", Stream, Date ("September 25, 2026"), *Pins published today* (number), **Mark target met** (fills in the stream's `target_pins_per_day`; disabled without a target), *Published with* (Another publishing tool / Published by hand → `source` external / manual), *Note*, Cancel / Save (+ **Delete** when the day already has an entry).
   * **Every cell is clickable** (TASK-FIX-053). A future cell opens **"Expected publishing"**: *Pins expected*, *Scheduled with*, *Note*, **Save as expected** (+ Delete). It is never saved or shown as published. When today's cell holds an expected entry, the modal becomes **"Confirm external publishing"**: Status "Expected externally — not confirmed", **Confirm as published** (primary), **Keep as expected**, Delete. Cell accessible names end with "Record publishing activity", "Plan external publishing" or "Confirm expected publishing".
   * An expected day's level is measured on planned + expected Pins (a forecast: its tooltip says "Expected externally: 5 (not confirmed)" and "Target met (forecast)"). The row shows "◌ Expected externally: N · planned in another tool, not confirmed yet" under the confirmed line; the two are never added together.
...
* Copy Markdown / Copy HTML / Download .md buttons (`components/wordpress/copy-export-buttons.tsx`) — shown whether or not the Project has a WordPress connection (since TASK-FIX-052; before, the Publish control replaced them when a site was connected). Below them, an "Include internal links" checkbox, on by default, enabled only when a WordPress site is connected (otherwise disabled with "Connect a WordPress site to this project to add internal links."). With the option on, Copy Markdown / Copy HTML call `POST /api/wordpress/[id]/export`: both copy buttons are disabled and the clicked one reads "Preparing export with internal links…" with a spinner (plus a loading toast); Download .md and the Publish control stay available. Then the result is copied (ClipboardItem with a pending blob to keep the click's user activation, `writeText` fallback) and the toast becomes "HTML copied with 3 internal links." / "Markdown copied without internal links.". No published post → info toast "No published articles available for internal linking. The export was copied without internal links."; no relevant post → "No relevant published articles found for internal linking. The export was copied without internal links."; WordPress or request failure → the original export is copied and a warning toast "Internal links could not be loaded. The original export was copied.". With the option off, the original export is copied locally with no request. Download .md always downloads the original Markdown. Nothing is computed while the page renders, and the stored article is never modified. Messages live in `lib/wordpress/export-copy-feedback.ts`
* Featured image (if generated)
* Meta description
* Quality report (`components/wordpress/article-quality-report.tsx`, TASK-FIX-046) — read-only card between the export/publish controls and the category editor: a Passed / Warnings / Issues found badge, a "N of M checks passed · X warnings · Y issues" summary with an "informational only" note, the Issues and Warnings lists, and a native `<details>` "All checks (M)" list (label, status icon + screen-reader status, message). No link or action — it never blocks export or publishing. Articles without a stored report show a dashed card "No quality report for this article". Collapsible since TASK-FIX-048 (`components/wordpress/quality-report-disclosure.tsx`, the only client part): the "Quality report" title is a toggle button (`aria-expanded`, `aria-controls`); the badge and summary stay visible, the lists are hidden when collapsed. Default: open for warnings, issues or no report; collapsed when every check passed. The viewer's choice is remembered per `generationId` in localStorage (`omniflow:quality-report-open:<id>`, read through `useSyncExternalStore`, in-memory fallback). Export, copy, category save and publish are separate components above/below the card and always available. Labels come from `lib/wordpress/quality-report-view.ts`
...
After a successful Save as Draft / Publish Now / Schedule, an info toast "N internal links added to older posts" appears when automatic internal links were inserted (TASK-FIX-051); internal-link problems (posts not loaded, URLs ignored) show as warning toasts like the Rank Math ones — never blocking. The article stored in OmniFlow and the reading view are unchanged: links are added only to the content sent to WordPress (and, since TASK-FIX-052, to the copied export when "Include internal links" is on).
