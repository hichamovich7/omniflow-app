...
TASK-FIX-053 (Publishing coverage — expected external activity on future days: every coverage cell is clickable; a future day saves an `expected` entry — "Expected externally", hollow dashed marker, never shown or counted as published — that only improves that day's coverage forecast; once the day has come it can be confirmed as `published`, kept expected, edited or deleted; confirmed and expected counters are shown apart ("Confirmed externally" / "Expected externally")) is implemented locally, not committed. Migration **038** adds `content_stream_publishing_activity.status` (`published` default / `expected`, CHECK) — **it must be applied by hand in the Supabase SQL Editor**; until then the dashboard reads no manual activity and saves fail. RLS, grants and the unique stream/day key are unchanged; 035 and every other migration are untouched. New `DELETE /api/content-streams/[id]/publishing-activity?activityDate=`; `PUT` takes an optional `status` (future = expected, `published` refused on a future day). No `pins` change, no Pinterest call. TypeScript, ESLint (touched files), the new offline spec (26/26), related suites and the production build pass; full renderer 537/538 with the same pre-existing, unrelated Pinterest failure. `tests/renderer/wordpress-quality-report.spec.ts` no longer requires 037 to be the last migration. Live RLS and the real browser click (gated case) are not yet checked. See CHANGELOG.md "TASK-FIX-053". Do not commit automatically.

TASK-FIX-052 (WordPress export — internal links in Copy Markdown / Copy HTML: "Include internal links" option, on by default when a WordPress site is connected; on click, the new read-only `POST /api/wordpress/[id]/export` applies the TASK-FIX-051 selection to the HTML or Markdown export — `<a href>` or `[anchor](url)` links, never HTML in Markdown, tags looked up but never created — and the client copies the result; never computed while rendering `/wordpress/[id]`; stored article never modified; no posts / no relevant post / WordPress error → original export copied with an informative or warning toast) is implemented and committed. The copy buttons are now also shown when a site is connected. No migration, prompt, model, Rank Math, tag, image, Quality Gate, Pinterest or Social Content Studio change; no AI call. TypeScript, ESLint (touched files), the new offline spec (28/28), the WordPress internal-links and publish SEO specs (79/79) and the production build pass; full renderer 511/512 with the same pre-existing, unrelated Pinterest failure. Clipboard behavior in a real browser and a real WordPress site are not yet checked. See CHANGELOG.md "TASK-FIX-052".

TASK-FIX-051 (WordPress publish — automatic internal links: before Save as Draft / Publish Now / Schedule, the site's published posts are loaded through `/wp-json/wp/v2/posts` with summary `_fields` only, filtered to same-site canonical links and never the post being updated, scored deterministically on primary keyword, SEO keywords, title and H2/H3 words, categories and tags, and up to 3 / 5 links are inserted in body paragraphs by a text-node-only HTML tokenizer — never in headings, FAQ, images or existing links; `internalLinks` report in the publish response; never blocking) is implemented and committed. No migration, prompt, model, Rank Math, tag, image, Quality Gate, Pinterest or Social Content Studio change; no AI call. The stored article and the Copy Markdown / Copy HTML exports are unchanged (links only in the content sent to WordPress). TypeScript, ESLint (touched files), the new offline spec (43/43) and the production build pass; full renderer 483/484 with the same pre-existing, unrelated Pinterest failure. Offline tests only — not yet checked against a real WordPress site. See CHANGELOG.md "TASK-FIX-051".
...
TASK-FIX-042 (Operational Command Center — `/dashboard` rebuilt as a daily Command Center: Today's workspace focus, Content streams table, 14-day Publishing coverage, ranked Recommended next actions, Sunday analytics review routine, This week view) is implemented locally, not yet validated against a live Supabase environment. Migrations **033 `tasks`** and **034 `task_occurrences`** (Command Center Phase 2b + minimal 2c, founder-approved 2026-09-25) must be applied by hand in the SQL Editor before priorities / the Sunday review can be saved. Every existing KPI, Weekly Progress metric, Quick Action, Recent Activity and the credits badge is kept; values are real (coverage/missing pins from `pins.publish_date` and content stream targets, local-calendar days) or shown as "Not tracked yet" — `command-center-mock.ts` and the mocked Active Projects are removed. No Pinterest generation, provider, CSV, renderer or generation-route change; no `pinterest_accounts` table. TypeScript, ESLint (touched files), 55 new/updated offline tests and the production build pass. See docs/tasks/TASK-COMMAND-CENTER-PHASE-2.md §15 and CHANGELOG.md "TASK-FIX-042". Do not commit automatically.
...
* Goal: answer daily "which stream/board do I work on today, which board is covered and until when, where is the buffer short, when is the Sunday review due" from real data, keeping every existing metric and action.
* Allowed scope: dashboard page + components, `lib/dashboard/*`, new `tasks`/`task_occurrences` tables + `/api/tasks*` routes (founder decision), docs. Pinterest generation, AI providers, CSV, renderer and generation routes untouched.
* Success criteria met offline: `required_buffer = pins/day × buffer days`, `missing = max(0, required − planned)`; coverage date from real planned dates; a board covered until Oct 5 is On track with no recommendation; under-buffer streams are Needs content / Create now; the stream that runs out first is the focus; the Sunday review shows Due Sunday / Due soon / Overdue and stays Overdue until completed; created / planned / planned-date-passed / unscheduled are separate; no mock data; Today's Priorities persisted.
* Validation: `tsc` pass, ESLint pass, 55/55 new/updated offline specs (also under TZ=UTC and Europe/Paris), full Playwright 276 pass / 84 skipped (auth-gated) / 2 pre-existing unrelated Pinterest failures, `next build` pass, `git diff --check` clean.
* Record: docs/tasks/TASK-COMMAND-CENTER-PHASE-2.md §15.
...
* Follow-up (2026-09-23): importance `None` for Headline / Subtitle / CTA disables the element (not generated, not persisted — `text.<element>` is `null` — not rendered, no line consumed); exact text + `None` and all-three-disabled are rejected with 400. See `docs/CHANGELOG.md`, `docs/API.md`.
