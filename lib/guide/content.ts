...
export const guideSections: GuideSection[] = [
  {
    ...
    points: [
      ...
      'Published a stream\'s Pins with another tool today? Click today\'s cell in Publishing coverage, enter how many Pins you published (or click "Mark target met" to use the stream\'s Pins per day), add a note if you like, and Save. They count toward today\'s target and show as "Published externally", but never change your Created or Planned counts, your Pins or their dates. Saving again the same day updates the entry.',
      'Scheduled Pins in another tool for a later day? Click that future cell in Publishing coverage, enter how many Pins you expect and Save as expected. They show with a hollow dashed dot as "Expected externally" — never as published — and only improve that day\'s coverage forecast. When the day comes, click the cell again to Confirm as published, keep them as expected, or Delete the entry. Confirmed and expected counts are always shown apart.',
  ...
  },
  {
    ...
    points: [
      ...
      'From pins, the article is planned around one clear promise — what the selected pins announce to the reader — and must actually deliver it rather than repeat the pins\' hooks. Each pin\'s title, description, keywords, image text, recorded image style, board, board section and content stream are used as context (board, section and stream only set the theme; they are never written into the article as facts). A pin\'s own destination link is the only pin link the article may use, and no link is ever invented.',
      'From pins, you can also add one External URL (optional) — a source you want the article to cite. It is linked at most once, and only if it is relevant to the article. Leave it empty to keep the automatic, verified external source only.',
      ...
      'The article is stored as Markdown. Use Copy Markdown, Copy HTML, or Download .md to take it anywhere else. If the Project has a WordPress connection, a Publish control appears above those buttons — see "WordPress Publishing" below.',
      'With a WordPress connection, "Include internal links" (on by default) makes Copy Markdown and Copy HTML add links to your already-published posts, chosen exactly like when publishing (up to 3 for short articles, 5 for medium and long ones). The links are prepared only when you click — this can take a few seconds ("Preparing export with internal links…") — and the article saved in OmniFlow never changes. Markdown gets Markdown links, HTML gets HTML links. If your blog has no published article yet, none fits, or your site can\'t be reached, the original export is copied and a message tells you so. Untick the option to copy the article exactly as generated. Download .md always gives the original.',
    ],
  },
  {
    ...
    points: [
      ...
      'Each post is sent with the article\'s H1 as its title, OmniFlow\'s slug, the meta description as its excerpt, its category and up to 8 tags taken from your SEO Keywords, the keywords of the Pins the article was made from, and your main keyword (existing WordPress tags are reused, missing ones created). No tags are invented: with only one keyword you get one tag, and with none the post is sent without tags and a warning tells you so.',
      'If your site runs Rank Math, OmniFlow also fills its SEO title (the meta title), meta description and focus keyword for the post. The focus keyword is the keyword you typed — for an article made from Pins, the keyword of the Pinterest generation those Pins come from. If there is no reliable keyword, the focus keyword is left empty and a warning tells you so. If Rank Math is missing or refuses the update, the article is still published and a warning tells you so — you can fill those fields in WordPress or simply publish again.',
      'When you save, publish or schedule, OmniFlow also links up to 3 (short articles) or 5 (medium and long articles) of your site\'s already-published posts from the article body — chosen from your keyword, SEO keywords, section titles, category and tags, with an anchor taken from a sentence of the article. Links are never added to headings, the FAQ, images or text that is already linked, never twice to the same post, and only to posts of your own site (never to the post being updated). If no older post fits, the article is sent unchanged; if your site\'s posts can\'t be loaded, the article is still published and a warning tells you so. Copy Markdown / Copy HTML add the same kind of links only when "Include internal links" is ticked (see above).',
