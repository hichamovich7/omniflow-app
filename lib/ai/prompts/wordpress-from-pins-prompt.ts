...
export function buildWordPressFromPinsPrompt(ctx: FromPinsPromptContext) {
  ...
  const imagesInstruction =
    ctx.imageCount > 0
      ? `- images: exactly ${ctx.imageCount} internal image slot${ctx.imageCount > 1 ? 's' : ''} to place within the body — each with { placementMarker, prompt, altText }, where placementMarker is ${imageMarkerNames} (in that order). Each slot corresponds, in the same order, to one of the pins listed above (slot 1 → pin 1, slot 2 → pin 2, etc.) — its image is an existing pin image that will be reused as-is, not newly generated. Write "prompt" as a short description of that pin's own visual (echoing its title/description) — it is stored for reference only, not used to generate anything. Write "altText" so it accurately describes what that specific pin's image actually shows, in ${langName}.`
